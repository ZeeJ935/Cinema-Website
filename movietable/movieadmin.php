...
<body>
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "INSERT INTO Movieadmin (MovieName, ReleaseDate, Duration, Genre, Showtimes, AvgRating, description, ImageUrl) VALUES ('$name', '$date', '$duration', '$genre', '$showtimes', '$rating', '$description', '$image')";
        if (mysqli_query($conn, $sql)) {
            move_uploaded_file($_FILES["image"]["tmp_name"], "uploads/" . $_FILES["image"]["name"]);
            ?>
            <script>
                alert('Data inserted');
                window.location.href = '/adminmovies/movietable.php';
            </script>
            <?php
        } else {
            ?>
