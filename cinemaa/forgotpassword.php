...
<body>
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "SELECT user_id FROM userinfo WHERE email='$email' and phone_no = '$phoneno'";
        $result = mysqli_query($con, $sql);
        if (mysqli_num_rows($result) == 0) {
    ?>
            <script>
                alert("You dont have an account on our page.");
            </script>
            <?php
        } else {
            ...
            // Update the password for the user in the database
                ...
                $updateSql = "UPDATE userinfo SET password='$password', confirm_password ='$confirm_password'  
                WHERE email='$email' and phone_no='$phoneno'";
